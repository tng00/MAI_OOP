...
int main() {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (f == 'p') {
            ...
            cin >> side;
            shared_ptr<Pentagon<double>> p(new Pentagon<double>(side));
            // cin >> *p.get();
            array.push_back(p);
        } else if (f == 'h') {
            std::cout << "Side length: ";
            cin >> side;
            shared_ptr<Hexagon<double>> h(new Hexagon<double>(side));
            // cin >> *h.get();
            array.push_back(h);
        } else if (f == 'o') {
            std::cout << "Side length: ";
            cin >> side;
            shared_ptr<Octagon<double>> o(new Octagon<double>(side));
            // cin >> *o.get();
            array.push_back(o);
        } else {
